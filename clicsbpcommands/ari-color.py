...
def run(args):
    ...
    concepts = sorted(set([row["CONCEPT"] for row in data if row["TAG"] == "color"
            ]))
    ...
    with open(CLICS.dir.joinpath("output", "ari-color.tsv"), "w") as f:
        f.write("FAMILY_A\tFAMILY_B\tARI_COLOR\tAMI_COLOR\tBCUBES_COLOR\n")
